...
const GameCanvas: React.FC<GameCanvasProps> = ({ settings, onGameOver }) => {
  ...
  useEffect(() => {
    ...
    const updatePhysics = () => {
      ...
      let ax = 0;
      let ay = stats.ballMass * settings.gravity;

      if (!isBroken.current) {
        const fRubber = settings.rubberK * (dist - settings.naturalLen);
        const angle = Math.atan2(dy, dx);
        ax -= (fRubber * Math.cos(angle));
        ay -= (fRubber * Math.sin(angle));
      }
    ...
    const updateUI = () => {
      ...
      const isDanger = s.stretch > 1.0 || isBroken.current;
      ...
      if (uiRefs.tensionValue.current) uiRefs.tensionValue.current.textContent = isBroken.current ? 'SNAPPED' : `${Math.round(stretchPercent)}%`;
      if (uiRefs.tensionLabel.current) {
        uiRefs.tensionLabel.current.textContent = isBroken.current ? 'BROKEN' : isDanger ? 'WARNING' : isPerfect ? 'PERFECT' : 'TENSION';
    ...
    const draw = (alpha: number) => {
      ...
      // Rubber line (only if not broken)
      if (!isBroken.current) {
